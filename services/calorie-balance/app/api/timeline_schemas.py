...
class HourlyDataPoint(BaseModel):
    ...
    calories_burned_exercise: Decimal = Field(
        Decimal("0"), description="Exercise calories"
    )
...
class DailyDataPoint(BaseModel):
    ...
    calories_consumed: Decimal = Field(
        Decimal("0"), description="Total calories consumed"
    )
    calories_burned_exercise: Decimal = Field(
        Decimal("0"), description="Exercise calories"
    )
    ...
    trend_direction: Optional[str] = Field(
        None, description="Trend direction indicator"
    )
...
class WeeklyDataPoint(BaseModel):
    ...
    avg_daily_consumed: Decimal = Field(
        Decimal("0"), description="Average daily consumed"
    )
    ...
    total_weight_change: Optional[Decimal] = Field(
        None, description="Weight change for week"
    )
    active_days: int = Field(0, description="Days with recorded activity")
    goal_adherence_pct: Optional[Decimal] = Field(
        None, description="Goal adherence percentage"
    )
    weekday_patterns: Optional[Dict[str, Decimal]] = Field(
        None, description="Weekday patterns"
    )
...
class MonthlyDataPoint(BaseModel):
    ...
    avg_net_calories: Decimal = Field(..., description="Average net calories")
    total_weight_change: Optional[Decimal] = Field(
        None, description="Weight change for month"
    )
    active_days: int = Field(0, description="Days with recorded activity")
    weekly_consistency: Optional[Decimal] = Field(
        None, description="Weekly consistency score"
    )
    seasonal_factor: Optional[Decimal] = Field(
        None, description="Seasonal adjustment factor"
    )
...
class IntradayDataPoint(BaseModel):
    ...
    cumulative_balance: Decimal = Field(..., description="Cumulative balance")
    activity_intensity: Optional[str] = Field(
...
class RealTimeMetrics(BaseModel):
    ...
    calories_remaining: Optional[Decimal] = Field(
        None, description="Calories remaining today"
    )
    burn_rate_per_hour: Decimal = Field(Decimal("0"), description="Current burn rate")
    consumption_rate_per_hour: Decimal = Field(
        Decimal("0"), description="Current consumption rate"
    )
    goal_achievement_probability: Optional[Decimal] = Field(
        None, description="Goal achievement probability"
    )
    next_meal_recommendation: Optional[str] = Field(
        None, description="Next meal recommendation"
    )
...
class TimelineExportData(BaseModel):
    ...
    inline_data: Optional[Union[Dict, List, str]] = Field(
        None, description="Inline export data"
    )
