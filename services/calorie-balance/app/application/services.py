...
class CalorieEventService:
    ...
    def __init__(
        self, event_repo: ICalorieEventRepository, balance_repo: IDailyBalanceRepository
    ):
    ...
    async def batch_record_events(
        ...
        try:
            ...
            for day in affected_dates:  # renamed from date to avoid shadowing import
                await self._update_daily_balance(user_id, day)
    ...
    async def _update_daily_balance(self, user_id: str, date: DateType) -> None:
...
class CalorieGoalService:
    ...
    async def create_weight_loss_goal(
        ...
        try:
            ...
            metabolic_profile = (
                await self.metabolic_service.calculate_metabolic_profile(
                    user_id=user_id,
            ...
            end_date = None
            logger.info(
                f"End date calculation - target_weight_kg: {target_weight_kg}, user_weight_kg: {user_weight_kg}, weekly_weight_change_kg: {weekly_weight_change_kg}"
            )
            logger.info(
                f"Types: target_weight_kg: {type(target_weight_kg)}, user_weight_kg: {type(user_weight_kg)}, weekly_weight_change_kg: {type(weekly_weight_change_kg)}"
            )
            ...
            if target_weight_kg and user_weight_kg and weekly_weight_change_kg:
                ...
                logger.info(
                    f"Weight difference: {weight_diff} kg, Weeks needed: {weeks_needed}"
                )
                ...
                logger.info(
                    "Missing parameters for end_date calculation - will be null"
                )
    ...
    async def create_goal(
        ...
        try:
            ...
            if custom_calorie_target:
                ...
                gender_enum = (
                    GenderType(user_gender) if user_gender else GenderType.OTHER
                )
                activity_enum = (
                ...
                if goal_type == GoalType.WEIGHT_LOSS and weekly_weight_change_kg:
                    ...
                    weekly_calorie_deficit = abs(weekly_weight_change_kg) * Decimal(
                        "7700"
                    )  # Cal per kg
                    daily_deficit = weekly_calorie_deficit / 7
                    daily_target = metabolic_profile.tdee_calories - daily_deficit
                    ...
                    weekly_calorie_surplus = weekly_weight_change_kg * Decimal(
                        "7700"
                    )  # Cal per kg
            ...
            goal = CalorieGoal(
                ...
                goal_type=goal_type,
                daily_calorie_target=daily_target,
                daily_deficit_target=daily_target - Decimal("2000")
                ...
                is_active=True,
                ai_optimized=bool(
                    all([user_weight_kg, user_height_cm, user_age, user_gender])
                ),
                optimization_metadata={
                    "method": "parameter_passing_pattern",
                    "user_metrics_provided": bool(
                        all([user_weight_kg, user_height_cm, user_age, user_gender])
                    ),
    ...
    def _calculate_end_date(
        ...
        if target_weight_kg and user_weight_kg and weekly_weight_change_kg:
            ...
            logger.info(
                "Missing user_weight_kg or weekly_weight_change_kg for end_date calculation"
            )
        else:
            logger.info(
                f"Using provided target_date: {target_date} or no target_weight_kg: {target_weight_kg}"
            )
    ...
    async def update_goal(
        self, goal_id: str, user_id: str, updates: Dict[str, Any]
    ) -> Optional[CalorieGoal]:
        """Update an existing calorie goal with provided changes."""
        try:
            # Get all user goals and find the specific one
            all_goals = await self.goal_repo.get_user_goals(
                user_id, include_inactive=True
            )
            current_goal = next(
                (goal for goal in all_goals if str(goal.id) == goal_id), None
            )
            ...
            weekly_weight_change_kg = updates.get(
                "weekly_weight_change_kg", current_goal.weekly_weight_change_kg
            )
            ...
            updated_goal = CalorieGoal(
                ...
                goal_type=GoalType(
                    updates.get("goal_type", current_goal.goal_type.value)
                )
                if "goal_type" in updates
                else current_goal.goal_type,
                daily_calorie_target=Decimal(
                    str(
                        updates.get(
                            "daily_calorie_target", current_goal.daily_calorie_target
                        )
                    )
                ),
                daily_deficit_target=Decimal(
                    str(
                        updates.get(
                            "daily_deficit_target", current_goal.daily_deficit_target
                        )
                    )
                ),
...
class MetabolicCalculationService:
    ...
    def __init__(self, profile_repo: IMetabolicProfileRepository):
    ...
    async def _get_activity_multiplier(self, activity_level: ActivityLevel) -> Decimal:
...
class AnalyticsService:
    ...
    async def get_hourly_analytics(
        self, user_id: str, date: DateType, hours_back: int = 24
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return await self.analytics_repo.get_hourly_summary(
                user_id=user_id, target_date=date, hours_back=hours_back
            )
        except Exception as e:
    ...
    async def get_weekly_analytics(
        self, user_id: str, weeks_back: int = 12, include_patterns: bool = True
    ) -> List[Dict[str, Any]]:
    ...
    async def get_monthly_analytics(
        self, user_id: str, months_back: int = 12, include_yearly_trends: bool = True
    ) -> List[Dict[str, Any]]:
    ...
    async def get_balance_timeline(
        self, user_id: str, period: str = "week", include_goals: bool = True
    ) -> List[Dict[str, Any]]:
    ...
    async def get_intraday_analytics(
        self, user_id: str, date: DateType, resolution_minutes: int = 15
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            while current_time < end_time:
                ...
                slot_events = await self.event_repo.get_events_by_time_range(
                    user_id=user_id, start_time=current_time, end_time=slot_end
                )

                # Calculate slot metrics
                calories_consumed = sum(
                    e.value for e in slot_events if e.event_type == EventType.CONSUMED
                )
                calories_burned = sum(
                    e.value
    ...
    async def get_pattern_analytics(
        ...
        try:
            ...
            if analysis_type == "behavioral":
                patterns.extend(
                    await self._detect_eating_patterns(user_id, lookback_days)
                )
            elif analysis_type == "seasonal":
                patterns.extend(
                    await self._detect_seasonal_patterns(user_id, lookback_days)
                )
            elif analysis_type == "weekly":
                patterns.extend(
                    await self._detect_weekly_patterns(user_id, lookback_days)
                )
            elif analysis_type == "daily":
                patterns.extend(
                    await self._detect_daily_patterns(user_id, lookback_days)
                )

            # Filter by confidence threshold
            return [
                p for p in patterns if p.get("confidence_score", 0) >= min_confidence
            ]
    ...
    async def generate_real_time_analytics(
        ...
        try:
            ...
            current_balance = (
                today_balance.net_calories if today_balance else Decimal("0")
            )

            # Calculate rates from recent events (last hour)
            one_hour_ago = datetime.utcnow() - timedelta(hours=1)
            recent_events_filtered = [
                e for e in recent_events if e.event_timestamp >= one_hour_ago
            ]

            consumed_last_hour = sum(
                e.value
                for e in recent_events_filtered
            ...
            if include_predictions:
                # Add predictions
                metrics.update(
                    await self._generate_predictions(user_id, current_balance)
                )
    ...
    def _calculate_seasonal_factor(self, month: int) -> Decimal:
        ...
        seasonal_factors = {
            12: Decimal("1.1"),
            1: Decimal("1.1"),
            2: Decimal("1.05"),  # Winter
            3: Decimal("1.0"),
            4: Decimal("0.95"),
            5: Decimal("0.95"),  # Spring
            6: Decimal("0.9"),
            7: Decimal("0.9"),
            8: Decimal("0.9"),  # Summer
            9: Decimal("0.95"),
            10: Decimal("1.0"),
            11: Decimal("1.05"),  # Fall
        }
    ...
    def _calculate_activity_intensity(self, events: List[CalorieEvent]) -> str:
        ...
        total_activity = sum(
            e.value for e in events if e.event_type == EventType.BURNED_EXERCISE
        )
    ...
    async def get_daily_dashboard(self, user_id: str, date: DateType) -> Dict[str, Any]:
    ...
    async def _generate_daily_insights(
        self,
        daily: Optional[DailyCalorieSummary],
        ...
        if daily:
            ...
            if daily.active_hours < 8:
                insights.append("Try to spread calorie intake across more hours")
    ...
    async def _analyze_weekly_trends(
        ...
        for i in range(1, len(weeks)):
            ...
            if curr_week.week_end_weight and prev_week.week_start_weight:
                weight_change = curr_week.week_end_weight - prev_week.week_start_weight
                weight_change_trend.append(weight_change)

        return {
            "net_calorie_trend": "increasing"
            if sum(net_calorie_trend) > 0
            else "decreasing",
            "average_weight_change_per_week": (
                sum(weight_change_trend) / len(weight_change_trend)
                if weight_change_trend
                else None
            ),
