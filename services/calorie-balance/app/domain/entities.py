...
class CalorieEvent(BaseEntity):
    ...
    def validate_value_by_type(cls, v, values):
        ...
        elif (event_type in [EventType.BURNED_EXERCISE, EventType.BURNED_BMR]
              and v > 2000):
...
class CalorieGoal(BaseEntity):
    ...
    is_active: bool = Field(
        True, description="Whether goal is currently active"
    )
    
    # AI optimization
    ai_optimized: bool = Field(
        False, description="Whether goal is AI-optimized"
    )
...
class DailyBalance(BaseEntity):
    ...
    events_count: int = Field(
        0, ge=0, description="Number of events aggregated"
    )
...
class MetabolicProfile(BaseModel):
    ...
    bmr_calories: Decimal = Field(
        ..., gt=0, description="Basal Metabolic Rate"
    )
