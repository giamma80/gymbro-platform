...
async def register_user(request: RegisterRequest, http_request: Request):
    ...
    try:
        ...
        logger.info("User registered successfully", 
                   user_id=created_user.id, email=request.email)
...
async def login_user(request: LoginRequest, http_request: Request):
    ...
    try:
        ...
        # Verify password
        if not verify_password(request.password, credentials.password_hash, credentials.salt):
        ...
        logger.info("User logged in successfully", 
                   user_id=user.id, session_id=created_session.id)
...
async def request_password_reset(request: PasswordResetRequest, http_request: Request):
    ...
    try:
        ...
        logger.info("Password reset requested", 
                   user_id=user.id, email=request.email, token=reset_token)
...
async def confirm_password_reset(request: PasswordResetConfirmRequest, http_request: Request):
    """Confirm password reset with token."""
    try:
